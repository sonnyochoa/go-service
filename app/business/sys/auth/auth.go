...
func New(activeKID string, keyLookup KeyLookup) (*Auth, error) {
	...
	if err != nil {
		return nil, errors.New("active Key ID does not exist in store")
	}
	...
	keyFunc := func(t *jwt.Token) (interface{}, error) {
		keyIDRaw, ok := t.Header["kid"]
		if !ok {
			return nil, fmt.Errorf("kid missing from header: %w", err)
		}

		keyID, ok := keyIDRaw.(string)
		if !ok {
			return nil, fmt.Errorf("kid malformed: %w", err)
		}

		fmt.Printf(" ** ||===  KID: %s  ===|| **\n", keyID)
		return keyLookup.PublicKey(keyID)
	}
