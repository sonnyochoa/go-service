...
func TestAuth(t *testing.T) {
	t.Log("Given the need to be able to authenticate and authorize access.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a single user.", testID)
		{
			...
			t.Logf("\t%s\tTest %d:\tShould be able to parse the calims.", success, testID)

			if exp, got := len(claims.Roles), len(parsedClaims.Roles); exp != got {
				t.Logf("\t\tTest %d:\texp: %d", testID, exp)
				t.Logf("\t\tTest %d:\tgot: %d", testID, got)
				t.Fatalf("\t%s\tTest %d:\tShould have the number of expected roles: %v", failed, testID, err)
			}
			t.Logf("\t%s\tTest %d:\tShould have the number of expected roles.", success, testID)
